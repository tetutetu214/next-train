...
## 全社対応の知見（2026-06-11 Issue #7）
- 通常キーのフル提供（駅座標+時刻表）は7社・440駅で確定: メトロ186/都営149/横浜市営42/MIR20/多摩モノレール19/ゆりかもめ16/TWR8。残り18社はマスタのみ（直通行先の名前解決に利用）。
- **カレンダー区分は事業者・路線で粒度が違う**: 基本は Weekday/SaturdayHoliday の2区分だが、都電荒川線だけ Weekday/Saturday/Holiday の3区分（SaturdayHoliday レコードが存在しない）。実測: 平日178/土曜157/休日147本。検索側が実日種別から優先順位つきで区分を選ぶ必要がある（selectTimetablesByCalendar、方向ごとに独立選択）。
- カレンダーは StationTimetable をフィルタなしで取って worker 側で選択する方式に変更（区分の存在有無が路線ごとに違うため、フィルタ指定だと0件パターンを踏む）。
- ゆりかもめは TrainInformation 提供なし（0件）。バナーは「異常があるときだけ表示」設計なので壊れない。
...
- 2026-06-11 **カレンダー区分の事業者差**: ODPT の時刻表カレンダーは事業者・路線で粒度が違い、都電荒川線は Saturday/Holiday 別区分で SaturdayHoliday が無い。土曜に SaturdayHoliday で検索すると0件になるため、実日種別からの優先順位選択が必要。
- 2026-06-11 **フィルタなし取得+サーバ側選択**: 区分の存在有無がデータ側で揃っていない場合、決め打ちフィルタは0件パターンを踏む。全部取って優先順位選択が確実。方向ごとに提供状況が揃う保証もないため選択は方向単位で独立に行う。
- 2026-06-11 **出典表記はライセンス上の義務**: CC BY 4.0 等のクレジット表示は利用条件。対象事業者が増えたら表記も更新する。
...
- 2026-06-13 **スプリットフラップ（反転フラップ）演出の設計**: 数字(0〜9の有限集合)はフラップの「次面へ送る」演出が成立するが、漢字は文字集合が数千あって送り演出が破綻するため、行先・種別は1文字ずつフラップせず面ごと差し替える。1桁のCSS実装は「上半分・下半分の2枚パネルに分離し、中央のヒンジ軸で上半分だけが rotateX して倒れる」のが定番（軸が中央にあるため上下分離が本質的に必要）。prefers-reduced-motion オン時はアニメを止め即切り替え。
